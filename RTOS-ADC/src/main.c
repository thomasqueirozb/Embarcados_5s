...
void draw_screen(void) {
  ...
  ili9488_draw_filled_rectangle(0, 0, ILI9488_LCD_WIDTH - 1,
                                ILI9488_LCD_HEIGHT - 1);
}
...
void font_draw_text(tFont *font, const char *text, int x, int y, int spacing) {
  ...
  while (*p != NULL) {
    ...
    if (letter <= font->end_char) {
      ...
      ili9488_draw_pixmap(x, y, current_char->image->width,
                          current_char->image->height,
                          current_char->image->data);
...
void task_lcd(void) {
  ...
  while (true) {
    if (xQueueReceive(xQueueTouch, &(touch),
    ...
    if (xQueueReceive(xQueueADC, &(adc),
...
int main(void) {
  ...
  if (xTaskCreate(task_adc, "adc", TASK_LCD_STACK_SIZE, NULL,
                  TASK_LCD_STACK_PRIORITY, NULL) != pdPASS) {
