...
void task_adc(void) {
    ...
    if (xQueueReceive( xQueueADC, &(adc), 100)) {
        if(i > NUM_TAPS) {
            ...
            plot.raw = (uint) inputF32[0];
            plot.filtrado = (uint) outputF32[0];
            xQueueSend(xQueuePlot, &plot, 0);
            i = 0;
        } else {
            i++;
            inputF32[i]= (float32_t) adc.value;
        }
    }
}
...
int main(void) {
    ...
    TASK_CREATE(task_mxt, MXT);
    TASK_CREATE(task_lcd, LCD);
    TASK_CREATE(task_adc, ADC);
